...
export default function EditIceClient({ ice }: { ice: { id: string, name: string, description: string | null, instructions: string | null } }) {
    const router = useRouter()
    const [open, setOpen] = useState(false)
    const [name, setName] = useState(ice.name)
    const [description, setDescription] = useState(ice.description || "")
    const [instructions, setInstructions] = useState(ice.instructions || "")
    ...
    useEffect(() => {
        if (open) {
            ...
            setInstructions(ice.instructions || "")
        }
    }, [open, ice])
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`/api/ices/${ice.id}`, {
                method: "PUT",
            ...
            if (!res.ok) {
                ...
                throw new Error(text || "Failed to update ice")
            }
            ...
            >
                <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 bg-white/5 rounded-lg flex items-center justify-center group-hover:bg-[#00d2ff]/5 transition-colors">
                        <IceIcon type={ice.name} className="w-6 h-6 text-white/40 group-hover:text-[#00d2ff]/60 transition-colors" />
                    </div>
                    <div className="flex-1">
                        <div className="flex justify-between items-start">
                            <h4 className="font-bold text-white text-sm group-hover:text-[#00d2ff] transition-colors">{ice.name}</h4>
                            <i className="ph-fill ph-pencil-simple text-white/20 group-hover:text-[#00d2ff] transition-colors text-xs"></i>
                        </div>
                    </div>
                </div>
                <p className="text-xs text-[#888c94] mb-2">{ice.description}</p>
                {ice.instructions && (
                    <div className="bg-[#00d2ff]/5 border border-[#00d2ff]/10 rounded-lg p-2.5">
                        <p className="text-[10px] font-black uppercase text-[#00d2ff] mb-1 tracking-widest flex items-center gap-1.5">
                            <i className="ph-fill ph-info"></i> Usage Tips
                        </p>
                        <p className="text-xs text-[#888c94] leading-relaxed italic">{ice.instructions}</p>
                    </div>
                )}
            </div>
